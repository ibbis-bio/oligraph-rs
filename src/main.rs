...
fn main() {
    ...
    let gfa_path = PathBuf::from(format!("{}.gfa", prefix.display()));
    ...
    let fasta_out_path = PathBuf::from(format!("{}.fasta", prefix.display()));
    ...
    if !contigs.is_empty() {
        let contigs_path = PathBuf::from(format!("{}.contigs.fasta", prefix.display()));
